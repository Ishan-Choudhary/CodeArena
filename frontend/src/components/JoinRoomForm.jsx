...
export default function JoinRoomForm({ fetchProblemDetails }) {
  ...
  const handleSearchRoom = async (e) => {
    ...
    setRoomDetails(null);
    try {
      const res = await fetchWithAuth(`${import.meta.env.VITE_API_URL}/api/rooms/${code}/`);
      if (res.ok) {
  ...
  const handleJoinRoom = async () => {
    ...
    setJoining(true);
    try {
      const res = await fetchWithAuth(`${import.meta.env.VITE_API_URL}/api/rooms/${code}/join/`, {
        method: 'POST'
      });
